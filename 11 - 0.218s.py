...
CACHE_DIR = 'cache'
PARQUET_STORE = 'ultimate_pure_xgboost_degradation.parquet'
...
# =====================================================================
# 2. INGESTION & NATURAL DEGRADATION EXTRACTION
# =====================================================================

def fetch_sunday_race(year, race, drivers):
    """Ingests race telemetry and computes clean baseline degradation without artificial terms."""
    try:
        ...
        for sid in valid['Stint_ID'].unique():
            ...
            warmup_window = stint[(stint['TyreLife'] >= 2) & (stint['TyreLife'] <= 4)]
...
def build_season_dataset(races, drivers):
    if os.path.exists(PARQUET_STORE):
        print(f"[1/3] Loading clean dataset: {PARQUET_STORE}")
        return pd.read_parquet(PARQUET_STORE)
        
    print("[1/3] Compiling Full-Season Database...")
    data = []
    for race in races:
    ...
    print(f"  -> Saved {len(master)} pristine laps.")
    return master

# =====================================================================
# 3. HIGH-CAPACITY MONOTONIC XGBOOST (NO ARTIFICIAL KINKS)
# =====================================================================

def train_pure_monotonic_xgboost(df):
    """
    Trains a high-capacity XGBoost model with zero artificial feature engineering.
    Allows trees to dynamically map true thermal cliffs cleanly.
    """
    print("\n[2/3] Training High-Capacity Monotonic XGBoost Model...")
    
    features = ['TyreLife', 'Energy_Index', 'Compound_C_Rating', 'TrackTemp']
    X = df[features]
    y = df['Degradation_Delta']
    
    # Monotonic constraints: TyreLife (1), Energy_Index (1), Compound_C_Rating (1), TrackTemp (0)
    model = xgb.XGBRegressor(
        n_estimators=450,
        learning_rate=0.025,
        max_depth=6,              # Deep enough to capture sharp non-linear cliffs
        subsample=0.85,
        colsample_bytree=0.9,
        monotone_constraints=(1, 1, 1, 0),
        reg_lambda=1.0,           # Low regularization to accurately track sharp late-race drop-offs
        random_state=42,
        n_jobs=-1
    )
    
    model.fit(X, y)
    print("  -> Pure Monotonic XGBoost successfully trained.")
    return model, features

# =====================================================================
# 4. VALIDATION SUITE & LOGGING
# =====================================================================

def run_multi_track_validation(model, features, year, target_races, driver):
    """Validates predictions against real telemetry and outputs true performance metrics."""
    print("\n=====================================================================")
    ...
    for idx, target_race in enumerate(target_races):
        ...
        warmup_window = clean_stint[(clean_stint['TyreLife'] >= 2) & (clean_stint['TyreLife'] <= 4)]
        if warmup_window.empty:
        ...
        ax.plot(smooth_laps, stint_base_pace + calibrated_smooth_delta, 
                color='red', linewidth=3, zorder=5, label='Monotonic XGBoost')
    ...
    print(f" NEW MONOTONIC XGB MAE  : {final_mae:.3f}s / lap")
...
if __name__ == "__main__":
    ...
    model, features = train_pure_monotonic_xgboost(df_season)
